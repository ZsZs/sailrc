...
export class ImageUploadViewAdapter implements FormViewAdapter, AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit() {
    ...
    //   this.imageUpload.options.changes.subscribe(() => {
    //     Promise.resolve().then(() => this.imageUpload.writeValue(this.value));
    //   })
    // );
  }
...
//    this.imageUpload.registerOnTouched(fn);
  }

  setIsDisabled(isDisabled: boolean) {
    console.log('image-upload: setIsDisabled');
//    this.imageUpload.setDisabledState(isDisabled);
  }
}
